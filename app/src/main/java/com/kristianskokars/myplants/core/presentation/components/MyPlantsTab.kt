...
/** Modified version of Material3 tab, to remove the inner padding of tabs. */
@Composable
fun MyPlantsTab(
    selected: Boolean,
    onClick: () -> Unit,
    ...
    val styledText: @Composable (() -> Unit)? = text?.let {
        @Composable {
            val style = MaterialTheme.typography.bodyMedium.copy(textAlign = TextAlign.Start)
...
private fun TabTransition(
    ...
    val transition = updateTransition(selected, label = "TabSElected")
...
private fun Placeable.PlacementScope.placeTextAndIcon(
    ...
    val textOffset = with(density) {
        baselineOffset.roundToPx() + 3.0.dp.roundToPx()
    }
